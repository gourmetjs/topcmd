...
proto.getTargets = function() {
  ...
  return this._targets.filter(function(target) {
    if (self.options.dirs) {
      return true;  // this._targets was made from dirs
    } else {
      return target.pkg.scripts && target.pkg.scripts[self.options.command] !== undefined;
    }
  });
};
...
proto._readTargets = function() {
  ...
  _scan(this.root, this.options.dirs);
