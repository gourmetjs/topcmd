...
function TopCommander(options) {
  ...
  if (!options.runOptions) {
    this.argv = minimist(process.argv.slice(2), MINIMIST_OPTIONS);
    this.runOptions = this.parseArgv(this.argv);
  } else {
...
proto.buildCommandString = function(target) {
  ...
  if (target.pkg.scripts && target.pkg.scripts[this.runOptions.command])
    output.push("run");

  output.push(this.runOptions.command);

  if (this.runOptions.restArgv)
    output.push(this.runOptions.restArgv);
